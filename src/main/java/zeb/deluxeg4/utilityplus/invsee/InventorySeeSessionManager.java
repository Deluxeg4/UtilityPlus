...
public final class InventorySeeSessionManager {
    ...
    public void open(OfflinePlayer target, Player viewer) {
        ...
        refresh(session, () -> PaperFoliaTasks.runForPlayer(plugin, viewer, () -> {
            ...
            // Inventory views belong to the viewer's region on Folia. Keeping the
            // refresh trigger on that entity scheduler avoids touching a view from
            // the global scheduler when regions are ticking independently.
            session.refreshTask = PaperFoliaTasks.runForPlayerTimer(plugin, viewer, task -> {
                if (sessions.get(session.viewerId) != session || !viewer.isOnline()) {
                    task.cancel();
    ...
    private void refresh(Session session, Runnable after) {
        ...
        PaperFoliaTasks.runForPlayer(plugin, target, () -> {
            if (sessions.get(session.viewerId) != session || !target.isOnline()) {
                return;
            }
            ...
            PaperFoliaTasks.runForPlayer(plugin, viewer, () -> {
                if (!viewer.isOnline() || sessions.get(session.viewerId) != session) {
                    close(session.viewerId);
                    return;
                }
                // Do not let an in-flight refresh overwrite a click that is being
                // propagated to the target's region.
                if (session.applyingChanges) {
                    return;
                }
